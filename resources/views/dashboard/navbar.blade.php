        ...
        <div class="navbar-header">
            ...
            <ol class="breadcrumb hidden-xs">
                ...
                @for($i = 1; $i <= count(Request::segments()); $i++)
                    <?php $breadcrumb_url .= '/' . rawurlencode(Request::segment($i)); ?>
                    @if(Request::segment($i) != ltrim(route('voyager.dashboard', [], false), '/') && !is_numeric(Request::segment($i)))
                        ...
                        @if($i < count(Request::segments()) & $i > 0 && array_search('database',Request::segments())===false)
                            <li class="active"><a
                                        href="{{ $breadcrumb_url }}">{{ ucwords(str_replace("\x1e", ' @ ', strtr(Request::segment($i), ['-' => ' ', '_' => ' ']))) }}</a>
                            </li>
                        @else
                            <li>{{ ucwords(str_replace("\x1e", ' @ ', strtr(Request::segment($i),  ['-' => ' ', '_' => ' ']))) }}</li>
                        @endif
